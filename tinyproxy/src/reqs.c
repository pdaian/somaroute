...
struct buffer_s {/*two structs copied from buffer.c because it's not in that header and I want to confine edits to one file. jlasota*/
...
char_vector buffer_to_char_vector (struct buffer_s * buffptr) {/*jlasota*/
	char_vector toReturn;
	toReturn.length = buffptr->size + 1;
	toReturn.underlying = safemalloc(toReturn.length);
	size_t i = 0;
	if (!toReturn.underlying) {
		toReturn.length = 0;
		return toReturn;
	}
	struct bufline_s *line;
	size_t j;
	for (line = buffptr->head; line != NULL; line = line->next)
		for (j = line->pos; j < line->length; ++j, ++i)
			toReturn.underlying[i] = line->string[j];
	toReturn.underlying[buffptr->size] = '\0';/*In case buffptr string not null-terminated.*/
	return toReturn;
}
...
void debug_output (char* title, char* string) {
	char path [200];
	int i;
	FILE *f = NULL;
	for (i = 0; i < 50; ++i) {
		snprintf(path, 200, "/root/debugs/%s_%d.txt", title, i);
		if (!file_exists(path)) {
			f = fopen(path, "w");
			break;
		}
...
void debug_output_char_vector (char* title, char_vector vector) {
	char path [200];
	int i;
	FILE *f = NULL;
	for (i = 0; i < 50; ++i) {
	...
	for (i = 0; i < vector.length; ++i)
		fprintf(f, "%c", vector.underlying[i]);
	fclose(f);
}

int match_at (char_vector haystack, char* needle, size_t index, int caseSensitive) {
	size_t i = 0;
	for (; index + i < haystack.length; ++i) {
		if (needle[i] == '\0')
			return 1;
		char left = haystack.underlying[index + i];
		char right = needle[i];
		if (caseSensitive ? (left != right) : (tolower(left) != tolower(right)))
			return 0;
	}
	return 0;
}

int char_vector_index_of (char_vector haystack, char* needle, int caseSensitive) {
	int i;
	for (i = 0; i < haystack.length; ++i)
		if (match_at(haystack, needle, i, caseSensitive))
			return i;
	return -1;
}

int char_vector_count_within (char_vector haystack, char* needle, int caseSensitive) {
	int count = 0;
	int i;
	for (i = 0; i < haystack.length; ++i)
		if (match_at(haystack, needle, i, caseSensitive))
...
char_vector stringSurrounding (char_vector haystack, size_t index) {
	char_vector toReturn;
	for (toReturn.underlying = &haystack.underlying[index]; toReturn.underlying != &haystack.underlying[0] && *toReturn.underlying != '\0'; --toReturn.underlying);
	if (*toReturn.underlying == '\0')
		++toReturn.underlying;
	if (toReturn.underlying > &haystack.underlying[index])
		--toReturn.underlying;
	toReturn.length = strlen(toReturn.underlying);
	return toReturn;
}

void terminators_to_exes (char_vector cv) {
	size_t i;
	for (i = 0; i < cv.length; ++i)
		if (cv.underlying[i] == '\0')
			cv.underlying[i] = 'X';
}

#define LENGTH_OF_ARRAY(x) (sizeof(x) / sizeof(x[0]))

struct timeval get_delay (struct conn_s *connptr) {/*jlasota*/
	/*log_message(LOG_INFO, ";%d looking for the thing.", countup);*/
	size_t string_length = connptr->sbuffer->size;
	struct timeval toReturn;
	toReturn.tv_sec = 0;
	toReturn.tv_usec = 0;
	char_vector response_bytes;
	response_bytes = buffer_to_char_vector(connptr->sbuffer);
	if (response_bytes.underlying == NULL) {/*malloc failed.*/
		/*debug_output("malloc_fail", "");*/
		toReturn.tv_sec = 0;/*don't delay.*/
		return toReturn;
	}
	int htmlTagIndex = char_vector_index_of(response_bytes, "<html", 0);
	if (htmlTagIndex < 0) {
		terminators_to_exes(response_bytes);
		/*log_message(LOG_INFO, ";%d In %d chars, did not find the thing: %s", countup, (int) response_bytes.length, response_bytes.underlying);*/
		/*debug_output_char_vector("fail", response_bytes);*/
		return toReturn;/*Don't mess with this http response. It's not a page.*/
	}
	char_vector htmlString = stringSurrounding(response_bytes, htmlTagIndex);
	/*log_message(LOG_INFO, ";%d found the thing: %s", countup, htmlString.underlying);*/
	/*debug_output("succeed", string);*/
	...
	int good_count = 0;
	size_t i;
	for (i = 0; i < LENGTH_OF_ARRAY(bad_words); ++i)
		bad_count += char_vector_count_within(htmlString, bad_words[i], 0);
	for (i = 0; i < LENGTH_OF_ARRAY(good_words); ++i)
		good_count += char_vector_count_within(htmlString, good_words[i], 0);
	if (good_count - bad_count < 0) {
		log_message(LOG_INFO, "Because %d bad words outnumber %d good, delaying response to %s.", bad_count, good_count, connptr->client_ip_addr);
		toReturn.tv_sec = 5;/*delay.*/
	}
	else
		log_message(LOG_INFO, "Because %d good words are at least as many as %d bad, not delaying response to %s.", good_count, bad_count, connptr->client_ip_addr);
	safefree(response_bytes.underlying);
	return toReturn;
}

void asdf (int status, void* arg) {
	/*log_message(LOG_INFO, ";%d Satan has taken the thing from us.", countup);*/
}

static void relay_connection (struct conn_s *connptr)
{
		++countup;
		/*debug_output("relay_connection", "");*/
		/*log_message(LOG_INFO, ";%d beginning to relay what may contain the thing", countup);*/
        fd_set rset, wset;
        struct timeval tv;
        time_t last_access;
        int ret;
        ...
        for (;;) {
                ...
                if (ret == 0) {
                        ...
                        if (tdiff > config.idletimeout) {
                        		...
                        		/*debug_output("idle timeout", "");*/
                        		/*log_message(LOG_INFO, ";%d alas, a timeout prevented finding the thing", countup);*/
                                return;
                        } else {
                                continue;
                        }
                        ...
                        log_message (LOG_ERR,
                                     ...
                                     connptr->server_fd);
                		/*log_message(LOG_INFO, ";%d alas, a select error prevented finding the thing", countup);*/
                        return;
                } else {
                        /*
        ...
        struct timeval delay = get_delay(connptr);/*jlasota*/
		...
		/*debug_output("rc_finish", "");*/
		/*log_message(LOG_INFO, ";%d I am through looking for the thing", countup);*/
        return;
}
...
void handle_connection (int fd)
{
        ...
        for (i = 0; i < vector_length (config.add_headers); i++) {
                ...
                char* value = strcmp(header->name, "accept-encoding") ? header->value : "asdf";/*jlasota*/
